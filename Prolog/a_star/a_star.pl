...
a_star_rec(Result, OpenSet, ClosedSet, G, Path) :-
  ...
  findNewStates(NewNodes, node(State,F,Actions), G+1, ApplicableList),
...
findNewStates([node(NewState,NewF,NewAct)|NewNodes], node(State,F,Act), G, [Action|Actions]) :- % aggiorna la frontiera
  ...
  % COSì G+1 
  findNewStates(NewNodes, node(State,F,Act), G, Actions).
